...
public class Floor : MonoBehaviour {
    ...
    void Update() {
        float ang = Mathf.Atan2(lavadir.y, lavadir.x) + 1.1f * (Random.value * 2 - 1) * Time.deltaTime;
        lavadir = new Vector2(Mathf.Cos(ang), Mathf.Sin(ang));
        renderer.material.mainTextureOffset = renderer.material.mainTextureOffset + lavadir * Time.deltaTime * 0.03f;
        if (!inited)
            return;
